...
import {
  ...
  updateEstadoSolicitud, asignarCama, asignarCamaDirecta,
...
const crearValoracionMedicaSchema = z.object({
  ...
  frecuenciaCardiaca: z.string().optional().nullable(),
  frecuenciaRespiratoria: z.string().optional().nullable(),
...
function parseBody<T>(schema: z.ZodSchema<T>, body: unknown): T {
  ...
  if (!result.success) {
    console.log('ZOD ERRORS:', JSON.stringify(result.error.errors, null, 2));
...
router.post('/paciente/:id/asignar-cama', (req, _res, next) => {
  parseId(req.params.id);
  req.body = parseBody(
    z.object({
      camaId: z.coerce.number().int().positive('El ID de cama es obligatorio'),
